...
You can now listen to real-time player state updates such as playing, paused, stopped and completed.

This allows you to react to the current state of the audio — for example, showing a loader while buffering or updating UI controls when playback is paused.
...
void initState() {
  ...
  ApzAudioplayer().onPlayerStateChanged.listen((final PlayerState state) {
    setState(() {
      _currentState = state;
    });
  });
