...
class EncryptionDecryption {
    ...
    if (options.data != null &&
      ...
      try {
        ...
        if (tempSymmetricKeyPassBytes.isEmpty) {
          tempSymmetricKeyPassBytes = utf8.encode(
            _apzCrypto.generateRandomAlphanumeric(
        ...
        if (_symmetricKeyPass.isEmpty) {
          ...
          finalPayload["serverToken"] = encryptedKey;
        }
  ...
  /// The method checks for the presence of a `serverToken` to retrieve the
  /// symmetric key used for decryption. If the `serverToken` is present, it is
  ...
  Future<Response<dynamic>> decryptResponse(
    ...
    if (response.data != null &&
      ...
      try {
        ...
        final String? serverToken = responseData["serverToken"];
        if (serverToken != null && serverToken.isNotEmpty) {
          final String decryptedServerToken = await _apzCrypto
              .asymmetricDecrypt(
                privateKeyPath: _privateKeyPath,
                encryptedData: serverToken,
              );
          symmetricKeyBytes = utf8.encode(decryptedServerToken);
        ...
        if (_symmetricKeyPass.isEmpty) {
          throw DioException(
            ...
            response: response,
            error:
                """Failed to decrypt response data: serverToken missing or empty""",
          );
        }
